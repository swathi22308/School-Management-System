...
export default function Student() {
  ...
  const handleSaveClick = async () => {
    const updatedStudent = { ...editData };

    console.log("🛠 Updating student:", updatedStudent); 

    try {
      await axios.put(`/api/students/${updatedStudent.id}`, updatedStudent);
  ...
  const handleAddStudent = async () => {
    try {
      ...
      console.error("Failed to fetch updated student list:", err);
    }
  };
              ...
              <thead className="bg-gray-100">
                <tr>
                  {[
                    ...
                    <th key={i} className="py-2 text-left whitespace-nowrap">
                      {head}
                    </th>
                  ))}
                </tr>
                      ...
                      <td className="py-2 flex gap-2 whitespace-nowrap">
                        <Eye
                          className={`w-4 h-4 cursor-pointer ${
                            isHidden ? "text-gray-600" : "text-cyan-600"
                          }`}
                          onClick={() => {
                            window.location.href = `/layout/students/${s.id}`;
                          }}
                        />
